...
async def test_scrubber_created_as_slider_spanning_the_buffer(
    ...
    assert float(state.state) == 0.0  # default == live / newest (right end)
    attrs = state.attributes
    assert attrs["mode"] == "slider"
    # Timeline-oriented: 0 (right) = live, negative steps back into the past.
    assert attrs["min"] == -2  # 3 buffered frames -> 0, -1, -2
    assert attrs["max"] == 0
    assert attrs["step"] == 1
...
async def test_scrubbing_moves_the_selectable_frame_image(
    ...
    await _set(hass, -2)  # two steps back -> oldest buffered frame

    assert float(hass.states.get(_NUMBER).state) == -2.0
    img_state = hass.states.get(_FRAME_IMAGE)
    assert img_state.attributes["frame_offset"] == 2  # internal "frames back"
